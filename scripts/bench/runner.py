...
def parse(text: str, metrics: dict) -> dict:
    return {k: (c(m.group(1)) if (m := re.search(pat, text, re.M)) else None)
...
def main(bench: Bench, argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--sweep", required=True, metavar="KNOB=V1,V2",
                    help=f"axis to vary: {axes}")
    ...
    ap.add_argument("--max-vm-seconds", type=int, default=bench.max_vm_seconds,
                    metavar="SEC",
    ...
    for knob, (_env, parser) in bench.knobs.items():
        ap.add_argument(f"--{knob}", type=parser, default=None,
    ...
    if axis not in bench.knobs or not raw:
        raise SystemExit(f"--sweep must be one of {list(bench.knobs)}, e.g. conns=1,2,4")
    ...
    plan = ([(v, r) for r in range(1, a.reps + 1) for v in values] if a.interleave
