...
"""Plot bench results.
    ...
    just plot smoltcp-s3                        # sweep: throughput vs whichever knob varied
    just plot results/x/sweep-workers.csv --dark
    just plot talk                               # the fixed evaluation figures for the IDP talk

A sweep CSV records which knob was the axis, so one code path draws every
sweep. --series splits into one line per value of a column. A .md table is
written beside the .png so no value is reachable only by reading pixels.

'talk' is a different kind of thing: not a sweep, and not parameterized.
Stock matplotlib, default colour cycle, no theme -- it always regenerates the
same four named figures (counting, sampling, primitives, metal) from the
pmc-* result directories. It lives here rather than in its own script because
the two used to duplicate the same categorical palette by hand; sharing this
file makes that duplication a merge conflict instead of a silent drift.
"""
...
# ---------------------------------------------------------------------------
# Sweeps -- one line chart per CSV, axis and series read from the data
# ---------------------------------------------------------------------------
...
def sweep_main() -> int:
    ap = argparse.ArgumentParser(
        prog="plot.py",
        description="Plot a sweep CSV: throughput against whichever knob varied.",
    )
    ap.add_argument(
        "csv",
        type=Path,
...
# ---------------------------------------------------------------------------
# Talk figures -- fixed, unparameterized, always the same four named outputs
# ---------------------------------------------------------------------------
# Everything here is pooled across boots -- one boot cannot show between-boot
# spread, and on these instances the core clock alone moves ~14% from boot to
# boot -- so every value is a median over all reps of all boots and every
# error bar is the IQR over that same population.

# Validated categorical palette, in fixed slot order -- never cycled. Reuses
# INK's light-mode series rather than a second copy: matplotlib's default
# tab10 puts blue/orange/green in the first three slots, and green against
# orange is the classic red-green confusion, so these three were checked with
# the palette validator (worst adjacent pair dE 9.2 deutan, 27.6 normal) and
# kept as the one definition both the sweep charts and the talk figures draw
# from.
#
...
# ---------------------------------------------------------------------------
# Entry point -- 'talk' picks the fixed deck figures, anything else is a sweep
# ---------------------------------------------------------------------------
